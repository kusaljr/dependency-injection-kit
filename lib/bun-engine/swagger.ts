...
var generateHTML = function (
  ...
  var htmlWithCustomRobots = htmlWithCustomCss.replace(
    "<% robotsMetaString %>",
    robotsMetaString
  );
  var htmlWithFavIcon = htmlWithCustomRobots.replace(
    "<% favIconString %>",
    favIconString
  );
  ...
  const swaggerInitContent = _jsTplString
    .toString()
    .replace("<% swaggerOptions %>", stringify(initOptions));
  return {
    html: htmlWithCustomCssUrl.replace("<% title %>", customSiteTitle),
    swaggerInitJs: swaggerInitContent,
  };
};

// Global variable to hold the generated swaggerInit JS content.
// This is necessary because 'swagger-ui-init.js' is requested separately.
let globalSwaggerInitJs = "";
...
var setup = function (
  ...
  const { html, swaggerInitJs } = generateHTML(
    swaggerDoc,
    opts,
    options,
    customCss,
  ...
  globalSwaggerInitJs = swaggerInitJs; // Store the generated JS

  return function (reqPath) {
    if (reqPath === "/") {
      return { content: html, contentType: "text/html" };
    } else if (reqPath === "/swagger-ui-init.js") {
      return {
        content: globalSwaggerInitJs,
        contentType: "application/javascript",
      };
    } else {
      const swaggerDistPath = getAbsoluteSwaggerFsPath();
      const filePath = path.join(swaggerDistPath, reqPath);
    ...
    return null; // Indicates that the path is not handled by Swagger UI
  };
};
...
module.exports = {
  setup: setup,
  generateHTML: generateHTML, // Now returns an object with html and swaggerInitJs
};
